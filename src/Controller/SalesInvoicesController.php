...
class SalesInvoicesController extends AppController
{
	...
	public function report($id=null)
    {
		...
		->contain(['Companies', 'PartyLedgers'=>['Customers'], 'SalesLedgers', 'SalesInvoiceRows'=>['Items'=>['StockGroups'=>['ParentStockGroups'],'Sizes'], 'GstFigures']])
...
/* 		$items = $this->SalesInvoices->SalesInvoiceRows->Items->find()
		...
		if($receiptAccountLedgers)
		{
			...
			$partyOptions[]=['text' =>str_pad(@$Partyledger->customer->customer_id, 4, '0', STR_PAD_LEFT).' - '.$Partyledger->name, 'value' => $Partyledger->id ,'party_state_id'=>@$Partyledger->customer->state_id, 'partyexist'=>$receiptAccountLedgersName, 'billToBillAccounting'=>$Partyledger->bill_to_bill_accounting,'default_days'=>$Partyledger->default_credit_days,'customer_id'=>@$Partyledger->customer->customer_id];
		}
...
public function ajaxItemQuantityNew($itemId=null)
	...
	public function saleReturnIndex($id = null,$item_code = null)
    {
		...
		if(!empty($item_code)){
			...
			//	$InvoiceRows = $this->SalesInvoices->SalesInvoiceRows->find();
			//exit;
				/* $SalesInvoices = $this->SalesInvoices->find();
				$SalesInvoices->innerJoinWith('SalesInvoiceRows');
				$SalesInvoices->where(['SalesInvoices.company_id' => $company_id,'SalesInvoices.financial_year_id'=>$financialYear_id])->contain(['SalesInvoiceRowDatas'=>function ($q) use($item_id,$company_id) {
					return $q->where(['SalesInvoiceRowDatas.item_id' =>$item_id]);
				}]); */
			
			
				$SalesInvoices = $this->SalesInvoices->find()->contain(['PartyLedgers'])->where(['SalesInvoices.company_id' => $company_id,'SalesInvoices.financial_year_id'=>$financialYear_id]);
				...
				//pr($SalesInvoices->toArray()); exit;
				/* $SalesInvoices->innerJoinWith('SalesInvoiceRows')
				->group(['SalesInvoices.id'])
				->matching('SalesInvoiceRows.Items', function ($q) use($item_id,$company_id) {
											return $q->where(['Items.id' =>$item_id,'Items.company_id'=>$company_id]);
							})
				->contain(['Companies', 'PartyLedgers', 'SalesLedgers','SalesInvoiceRows'])
				->autoFields(true)
				->where(['SalesInvoices.company_id' => $company_id])->toArray(); */
